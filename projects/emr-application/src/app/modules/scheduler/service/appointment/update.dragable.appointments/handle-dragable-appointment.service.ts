...
export class HandleDragableAppointmentService {
  ...
  public handle(appointment: Appointment, events: CalendarEvent[], newStart: Date, newEnd: Date, start: number, end: number) {
  ...
  private handleSeries(events: CalendarEvent[]) {
    this.appointmentActionsService.promptDragSeriesAppointments(this.dialog, this.dragableAppointment,
       this.startDateBoundary,
       this.endDateBoundary).subscribe(result => {
      if (result.action !== 'cancel') {
        switch (result.selection) {
          case 'all':
            this.updateAllAppointments(result.appintmentsContainer, events);
            break;
          case 'one':
  ...
  private updateAllAppointments(appintmentsContainer: any, events: CalendarEvent[]) {
    // if (module === 'day') {
