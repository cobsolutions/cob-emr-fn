...
@Component({
  selector: 'daily-note-n',
  templateUrl: './daily-note-n.component.html',
  styleUrls: ['./daily-note-n.component.css']
})
export class DailyNoteNComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.dailyNoteService.get(this.noteId).subscribe((note: any) => {
      if (note) {
        ...
        if (note.subjective) {
          const subjectiveFormGroup = this.dailyNoteForm.get('subjective') as FormGroup;
          ...
          if (this.dailyNoteForm.get('subjective')?.get('basic')) {
            this.dailyNoteForm.get('subjective')?.patchValue(denormalizedSubjective);
        ...
        if (note.assessment) {
          ...
          if (Object.keys((this.dailyNoteForm.get('assessment') as FormGroup).controls).length > 0) {
            this.dailyNoteForm.get('assessment')?.patchValue(assessmentFormValue);
  ...
  setChildForm(section: string, formGroup: FormGroup) {
    ...
    if (section === 'objective' && this.pendingObjectiveData) {
      this.dailyNoteForm.get('objective')?.patchValue(this.pendingObjectiveData);
      this.pendingObjectiveData = null;
    } else if (section === 'subjective' && this.pendingSubjectiveData) {
      this.dailyNoteForm.get('subjective')?.patchValue(this.pendingSubjectiveData);
      this.pendingSubjectiveData = null;
    } else if (section === 'assessment' && this.pendingAssessmentData) {
      this.dailyNoteForm.get('assessment')?.patchValue(this.pendingAssessmentData);
      this.pendingAssessmentData = null;
  ...
  private buildMedicalNoteModel(): MedicalNoteRequest {
    const formRawData = this.dailyNoteForm as FormGroup
    console.log('formRawData  ', formRawData.getRawValue())

    // Get FormGroups and check if they have data
    const subjectiveGroup = this.dailyNoteForm.get('subjective') as FormGroup;
  ...
  draftAction(): Observable<any> {
    ...
    return this.dailyNoteService.draft(medicalNoteRequest, this.noteId);
  }
