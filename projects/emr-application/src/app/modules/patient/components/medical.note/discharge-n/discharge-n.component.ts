...
@Component({
  selector: 'discharge-n',
  templateUrl: './discharge-n.component.html',
  styleUrls: ['./discharge-n.component.css']
})
export class DischargeNComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.dischargeNoteService.get(this.noteId).subscribe((note: any) => {
      if (note) {
        ...
        if (note.subjective) {
          const subjectiveFormGroup = this.dischargeNoteForm.get('subjective') as FormGroup;
          ...
          if (this.dischargeNoteForm.get('subjective')?.get('basic')) {
            this.dischargeNoteForm.get('subjective')?.patchValue(denormalizedSubjective);
            this.pendingSubjectiveData = null;
          }
        }

        if (note.objective) {
          const objectiveFormGroup = this.dischargeNoteForm.get('objective') as FormGroup;
          ...
          const profileExists = this.dischargeNoteForm.get('objective')?.get('profile');
          const inspectionInitialized = this.dischargeNoteForm.get('objective')?.get('inspection')?.get('patient_consent');
          if (profileExists && inspectionInitialized) {
            this.dischargeNoteForm.get('objective')?.patchValue(denormalizedObjective);
        ...
        if (note.assessment) {
          ...
          if (Object.keys((this.dischargeNoteForm.get('assessment') as FormGroup).controls).length > 0) {
            this.dischargeNoteForm.get('assessment')?.patchValue(assessmentFormValue);
        ...
        if (note.billing) {
          ...
          // Denormalize true/false to yes/no
          const billingFormGroup = this.dischargeNoteForm.get('billing') as FormGroup;
  ...
  setChildForm(section: string, formGroup: FormGroup) {
    ...
    if (section === 'objective' && this.pendingObjectiveData) {
      this.dischargeNoteForm.get('objective')?.patchValue(this.pendingObjectiveData);
      this.pendingObjectiveData = null;
    } else if (section === 'subjective' && this.pendingSubjectiveData) {
      this.dischargeNoteForm.get('subjective')?.patchValue(this.pendingSubjectiveData);
      this.pendingSubjectiveData = null;
    } else if (section === 'assessment' && this.pendingAssessmentData) {
      this.dischargeNoteForm.get('assessment')?.patchValue(this.pendingAssessmentData);
      this.pendingAssessmentData = null;
  ...
  private buildMedicalNoteModel(): MedicalNoteRequest {
    const formRawData = this.dischargeNoteForm as FormGroup
  ...
  draftAction(): Observable<any> {
    ...
    return this.dischargeNoteService.draft(medicalNoteRequest, this.noteId);
  }
