...
@Component({
  selector: 'progress-note-n',
  templateUrl: './progress-note-n.component.html',
  styleUrls: ['./progress-note-n.component.css']
})
export class ProgressNoteNComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.progressNoteService.get(this.noteId).subscribe((note: any) => {
      if (note) {
        ...
        if (note.subjective) {
          const subjectiveFormGroup = this.progressNoteForm.get('subjective') as FormGroup;
          ...
          if (this.progressNoteForm.get('subjective')?.get('basic')) {
            this.progressNoteForm.get('subjective')?.patchValue(denormalizedSubjective);
            this.pendingSubjectiveData = null;
          }
        }

        if (note.objective) {
          const objectiveFormGroup = this.progressNoteForm.get('objective') as FormGroup;
          ...
          const profileExists = this.progressNoteForm.get('objective')?.get('profile');
          const inspectionInitialized = this.progressNoteForm.get('objective')?.get('inspection')?.get('patient_consent');
          if (profileExists && inspectionInitialized) {
            this.progressNoteForm.get('objective')?.patchValue(denormalizedObjective);
        ...
        if (note.assessment) {
          ...
          if (Object.keys((this.progressNoteForm.get('assessment') as FormGroup).controls).length > 0) {
            this.progressNoteForm.get('assessment')?.patchValue(assessmentFormValue);
        ...
        if (note.planOfCare) {
          ...
          // Check if form is already set up
          if (Object.keys((this.progressNoteForm.get('planOfCare') as FormGroup).controls).length > 0) {
            this.progressNoteForm.get('planOfCare')?.patchValue(planOfCareFormValue);
        ...
        if (note.billing) {
          ...
          // Denormalize true/false to yes/no
          const billingFormGroup = this.progressNoteForm.get('billing') as FormGroup;
          ...
          // Check if form is already set up
          if (Object.keys((this.progressNoteForm.get('billing') as FormGroup).controls).length > 0) {
            this.progressNoteForm.get('billing')?.patchValue(denormalizedBilling);
  ...
  setChildForm(section: string, formGroup: FormGroup) {
    ...
    if (section === 'objective' && this.pendingObjectiveData) {
      this.progressNoteForm.get('objective')?.patchValue(this.pendingObjectiveData);
      this.pendingObjectiveData = null;
    } else if (section === 'subjective' && this.pendingSubjectiveData) {
      this.progressNoteForm.get('subjective')?.patchValue(this.pendingSubjectiveData);
      this.pendingSubjectiveData = null;
    } else if (section === 'assessment' && this.pendingAssessmentData) {
      this.progressNoteForm.get('assessment')?.patchValue(this.pendingAssessmentData);
      this.pendingAssessmentData = null;
    } else if (section === 'planOfCare' && this.pendingPlanOfCareData) {
      this.progressNoteForm.get('planOfCare')?.patchValue(this.pendingPlanOfCareData);
      this.pendingPlanOfCareData = null;
    } else if (section === 'billing' && this.pendingBillingData) {
      this.progressNoteForm.get('billing')?.patchValue(this.pendingBillingData);
      this.pendingBillingData = null;
    }
  }
  ...
  private buildMedicalNoteModel(): MedicalNoteRequest {
    const formRawData = this.progressNoteForm as FormGroup
    console.log('formRawData  ', formRawData.getRawValue())

    // Get FormGroups and check if they have data
    const subjectiveGroup = this.progressNoteForm.get('subjective') as FormGroup;
    const objectiveGroup = this.progressNoteForm.get('objective') as FormGroup;
    const assessmentGroup = this.progressNoteForm.get('assessment') as FormGroup;
    const planOfCareGroup = this.progressNoteForm.get('planOfCare') as FormGroup;
    const billingeGroup = this.progressNoteForm.get('billing') as FormGroup;
  ...
  draftAction(): Observable<any> {
    ...
    return this.progressNoteService.draft(medicalNoteRequest, this.noteId);
  }
