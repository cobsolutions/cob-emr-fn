  ...
  ngOnInit(): void {
    this.medialNoteService.findROMTests(this.testName).subscribe(tests => {
      ...
      this.movementOptions.forEach(movement => {
        ...
        this.movementForm.get(`right_${movement.name}`).valueChanges.subscribe(v => {
          this.form.get(this.parentFieldName).setValue(this.movementForm.value);
        })
        this.movementForm.get(`left_${movement.name}`).valueChanges.subscribe(v => {
          this.form.get(this.parentFieldName).setValue(this.movementForm.value);
        })
      });
    })
