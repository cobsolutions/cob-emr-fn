...
export class SubjectiveMapperService {
  ...
  /**
  * Converts form raw value to Subjective model for backend
  */
  toModel(formValue: any): Subjective {
    ...
    // Map basic section
    if (formValue.basic) {
      subjective.basic = this.mapBasicToModel(formValue.basic);
    }

    // Map pain section
    if (formValue.pain) {
      subjective.pain = this.mapPainToModel(formValue.pain);
    }

    // Map priorFunction section
    if (formValue.priorFunction) {
      subjective.priorFunction = this.mapPriorFunctionToModel(formValue.priorFunction);
    }

    // Map currentFunction section
    if (formValue.currentFunction) {
      subjective.currentFunction = this.mapCurrentFunctionToModel(formValue.currentFunction);
    }

    // Map medicalHistory section
    if (formValue.medicalHistory) {
      subjective.medicalHistory = this.mapMedicalHistoryToModel(formValue.medicalHistory);
    }
  ...
  /**
     * Converts DTO from backend (camelCase) to form value object (snake_case)
     */
  fromDto(dto: Subjective): any {
    ...
    if (dto.priorFunction) {
      formValue.priorFunction = this.mapPriorFunctionFromDto(dto.priorFunction);
    }
  ...
  private mapBasicToModel(basic: any): Basic {
    const mapped: Basic = {
      ...
      treatmentSide: basic.treatment_side,
  ...
  private mapBasicFromDto(basic: any): any {
    const mapped: any = {
      ...
      treatment_side: basic.treatmentSide,
   ...
   */
  private mapPriorFunctionToModel(priorFunction: any): PriorFunction {
    // Convert from mixed kebab-snake format to camelCase
    // Example: prior-level-function_self-care_hygiene -> priorLevelFunctionSelfCareHygiene
    const camelCased = this.convertObjectFromMixedToCamelCase(priorFunction);
    const mapped: PriorFunction = { ...camelCased };

    // Handle conditional fields
    if (!mapped.priorLevelFunctionOther) {
      mapped.priorLevelFunctionOtherText = undefined;
    }
  ...
  /**
   * Maps priorFunction section from DTO (camelCase) to form (mixed kebab-snake format)
   */
  private mapPriorFunctionFromDto(priorFunction: any): any {
    // Set boolean flags based on presence of text fields
    if (priorFunction.priorLevelFunctionOtherText) {
      priorFunction.priorLevelFunctionOther = true;
    }

    // Convert all keys to mixed kebab-snake format
    // Example: priorLevelFunctionSelfCareHygiene -> prior-level-function_self-care_hygiene
    const mapped = this.convertObjectToMixedKebabSnakeCase(priorFunction);
